...
if __name__=="__main__":
	...
	print(f"Radial range is {fts.r_range} with length {len(fts.r_range)}.", flush=True)
	print(f"Rmax is {fts.Rmax} and dr is {fts.dr}.", flush=True)
	print(f"The length of box is {fts.L}.", flush=True)
	print(f"The discretization is {fts.ngrid_dim}.", flush=True)
	print(f"Number of grid elemnts is {fts.tot_grid}.", flush=True)
	print(f"Size of rho field is {RHO_FIELDS[0].shape}", flush=True)
	print(f"Number of stored fields is {len(RHO_FIELDS)}.", flush=True)
	print(f"Number of discrete points along an axis is {len(fts.x)}.", flush=True)
	print(f"Bin indices are {fts.bin_indices}.", flush=True)

	# set up an accumulator array
	xx, yy, zz  = xp.meshgrid(fts.x, fts.x, fts.x)
	r           = xp.sqrt(xx**2 + yy**2 + zz**2)
	bin_edges   = xp.arange(0, fts.Rmax + fts.dr, fts.dr)
	bin_centers = 0.5 * (bin_edges[:-1] + bin_edges[1:])
	r_flat      = r.ravel()
	print(f"Length of r_flat is {len(r_flat)}.", flush=True)
	rel_idx     = xp.where(r_flat<1e-12)[0][0]
	nrolls      = 100
	gr_store    = xp.zeros((len(RHO_FIELDS)*nrolls, len(bin_centers)), dtype=complex)

	# loop through the different fields and calculate the radial distribution function
	for idx, rho in enumerate(RHO_FIELDS):
