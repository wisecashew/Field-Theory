...
def probe_compute(fts_file):
	...
	rg_err  = np.std (np.array(rg_store)[-50:])/np.sqrt(5)
	rg_mean = np.mean(np.array(rg_store)[-50:])
...
if __name__=="__main__":
	...
	for tv,tp in zip(temp_vals, temp_paths):
		...
		try:
			...
			print(f"{tv} does not work. Moving on...", flush=True)
